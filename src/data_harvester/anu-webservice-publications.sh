...
# Retrieve publication information from the Java Service:
sudo wget -O publications.output --header='Accept: application/xml' http://localhost:9080/services/rest/person/publications/$invalue

# Update the publications data to the NLA Party Identifier database:
sudo java ANUJavaServicePublications $invalue|sort|uniq